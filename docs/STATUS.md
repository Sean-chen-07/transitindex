...
| [metric-set-build-plan.md](planning/metric-set-build-plan.md) | in-progress | **v1.1, 2026-07-01.** Phases 1 (rate only 5 hero metrics), 2 (revenue-boundary dictionary pass), the rename (`passenger_fare_revenue`→`farebox_revenue`, `operating_revenue`→`total_revenue_excluding_subsidy`, `total_operating_subsidy`→`subsidy`, migration 018), 4+5 (10 financial-statement additions + closed identities, migration 019), and 3 (`cost_basis` dimension: contract + migration 020 + repo/postgres/memory + derived-ratio operating-basis normalization + farebox_revenue numerator on `average_fare`/`farebox_recovery_ratio` + extractor tool schema) are built and tested. Remaining: Phase 6 (drop `fleet_capacity` → 4-class fleet composition) and Phase 7 (gold fixtures, OTP badge, final docs pass). Parity guardrail (refdata↔04_metrics.sql↔yaml) holds across all landed phases. |
