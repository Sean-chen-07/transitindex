...
| [balance-sheet-and-frequency-plan.md](planning/balance-sheet-and-frequency-plan.md) | in-progress | PSAB balance-sheet family + native monthly/quarterly frequency + carry-forward + workbook + web balance-sheet display. Ingest/DB shipped (11 metrics, migration 014, ratios). **2026-06-10:** web balance-sheet shipped as Section B of the Financials tab; §6's EN+FR statement anchors, printed_label/table_reference tool fields, and PSAB identities in flags.py (run at staging via validate_cohort in run_pdf) all landed. **2026-07-01:** §2 carries a superseding pointer — `debt_to_assets`/`net_debt_per_capita` ranking is retired by metric-set-build-plan.md Phase 1 (5-hero-metric allow-list); both now `comparable_flag=False`. **Gaps:** carry-forward web display; gold fixtures need real verified values; prior_value (yoy) lookup not wired; 6-sheet workbook superseded by per-agency tabs. |
...
| [metric-standards-review.md](planning/metric-standards-review.md) | living | **New 2026-06-14; design review, no product code changed.** First-principles audit of all 32 metrics in `refdata.METRICS` against CUTA/NTD/PSAB. Part A: per-metric standards table + reasoning; Part B: statement reconstruction + identity pass/fail (balance sheet closes; operating statement can't — missing amortization/other/annual-result); Part D: proposed additions (amortization, annual_surplus_deficit, total_revenue/expenses enterprise lens, other_operating_expenses, asset_consumption_ratio). **User decisions taken 2026-06-14** (in the "Decisions taken" block, and the body reconciled to match): rate only the 5 hero boxes (retires the 2 balance-sheet ranked ratios); drop `fleet_capacity`+weights → non-ranked 4-class fleet composition (Bus·Light rail·Heavy rail·Commuter rail, counted by trains); pin `operating_expenses` (amortization excluded + `cost_basis` field) and `operating_revenue` (earned only). Open: OTP rank badge (footnote vs drop). Implementation in metric-set-build-plan.md, **now fully landed (2026-07-01)** including the addendum v1.1 extraction-grade definition pass (entity_scope + component-vs-total confusions on every financial metric). |
| [metric-set-build-plan.md](planning/metric-set-build-plan.md) | done | **v1.1, built through Phase 7 (2026-07-01).** Phases 1 (rate only 5 hero metrics), 2 (revenue-boundary dictionary pass), the rename (`passenger_fare_revenue`→`farebox_revenue`, `operating_revenue`→`total_revenue_excluding_subsidy`, `total_operating_subsidy`→`subsidy`, migration 018), 4+5 (10 financial-statement additions + closed identities, migration 019), 3 (`cost_basis` dimension: contract + migration 020 + repo/postgres/memory + derived-ratio operating-basis normalization + farebox_revenue numerator on `average_fare`/`farebox_recovery_ratio` + extractor tool schema), 6 (drop `fleet_capacity` + `MODE_CAPACITY_WEIGHT`/`capacity_weight` column, migration 021; `refdata.FLEET_CLASS` mode→class map; `fleet_size` dictionary entry re-guided to count rail by TRAIN with a data caveat on existing car-counted values; web hero grid 6→5 boxes + a new non-ranked 4-class fleet-composition block wired end-to-end), and **7 — the extraction-grade definition pass** (addendum #1/#4/#5): every one of the 41 `metric_dictionary.yaml` entries now carries a component-vs-total confusion line; all 26 currency metrics carry an `entity_scope` (company-wide/whole-organization, transit-division-schedule caveat) and a `scale_note` ($000s/$M → whole-CAD); `subsidy`'s entry explicitly rejects promoting a single government level/program to the total; both fields flow automatically into the PDF-extraction prompt via `dictionary.extraction_guidance()` (already wired into `chunked_hybrid.py`/`claude_pdf.py`, confirmed unchanged this phase); a new `translink_annual_2024` gold fixture (`ingest/tests/fixtures/gold/`, `ingest/tests/test_gold_translink.py`) covers all 10 Phase-4 additions on a clean, fully-reconciling PSAB statement plus a should_flag case for the provincial-grant-vs-combined-subsidy trap. `data-dictionary.md` regenerated. 412 ingest tests green (409 baseline + 3 new). Remaining/deferred by design: the OTP rank-badge sub-decision (still open, no code change — see the doc's "Still open" note) and the pre-Phase-4/5 §2/§4 metric-name-map rows in `detail-view-metrics.md` (out of this phase's scope — web placement map, not the dictionary). Parity guardrail (refdata↔04_metrics.sql↔yaml) holds. |
...
| [source-registry.md](reference/source-registry.md) | in-progress | Maps each launch agency × metric to its source + license/tier + adapter build order. Metric catalog shipped (32 codes); agency-source matrix + adapter roadmap + license tables largely unbuilt. **2026-07-22:** FTA NTD rows (NTD-M `8bui-9xvu` monthly, NTD-A `ekg5-frzt` annual, US-federal public domain) added and their adapters BUILT (`ntd_monthly.py`/`ntd_annual.py`, migration 022, `ntd-monthly`/`ntd-annual` CLI, USD rank basis @ fixed 0.70 CAD→USD); the ~700-agency US seed itself is generated by `ingest/scripts/generate_ntd_agencies.py` and still needs its first network-enabled run (see TODOS.md data-population list). |
