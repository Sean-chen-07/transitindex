...
| **NTD-M** | FTA NTD Complete Monthly Ridership (Socrata `8bui-9xvu`) | All US Full Reporters (~700) | **US federal public domain (17 U.S.C. §105)** — no copyright, commercial use OK, attribution to "Federal Transit Administration, National Transit Database" is courtesy not obligation. *(License verified 2026-07-22 via secondary sources; the data.transportation.gov terms page itself was unreachable from the build sandbox — re-confirm on first fetch.)* | CSV + SODA API | https://data.transportation.gov/resource/8bui-9xvu | Monthly UPT/VRM/VRH per agency × mode × TOS, 2002→present, updated ~weekly. Adapter `ntd_monthly` sums to system-wide. Feed `ntd_monthly`, tier 0, enabled. |
| **NTD-A** | FTA NTD Annual Data — Metrics (Socrata `ekg5-frzt`) | All US Full Reporters (~700) | Same US-federal public domain as NTD-M | CSV + SODA API | https://data.transportation.gov/resource/ekg5-frzt | Annual fare revenues, total operating expenses (operating basis — depreciation excluded), UPT, VRM, VRH per agency × report year × mode × TOS. Adapter `ntd_annual`. Feed `ntd_annual`, tier 0, enabled. NTD has **no balance sheet** — the PSAB metric family stays Canada-only. Subsidy: future adapter over funding-sources dataset `4tmr-gwuu`. |
